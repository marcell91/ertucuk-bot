...
module.exports = async function Camera(client, state, ertu, action) {
    ...
    try {
        logChannel.send({
            embeds: [
                new EmbedBuilder({
                    ...
                    description: [
                        `${state.member} adlı üye ${state.channel} ${action ? 'kamerasını kapattı.' : 'kamerasını açtı.'}`,
                        '',
        ...
        client.logger.error('@Camera ->', error);
    }
}