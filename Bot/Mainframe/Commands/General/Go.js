...
module.exports = {
    ...
    messageRun: async (client, message, args, ertu, embed) => {
        ...
        if (message.member.permissions.has(Flags.MoveMembers) || ertu.settings.moveAuth.some((role) => message.member?.roles.cache.has(role))) {
            ...
            return embed.setDescription(`Başarıyla ${member} adlı kullanıcının odasına gidildi.`);
        } else {
            ...
            const question = await message.reply({
                ...
                embeds: [
                    new EmbedBuilder({
                        description: `${message.member} olduğunuz kanala gelmek istiyor. Kabul ediyor musunuz?`,
            ...
            collector.on('collect', async i => {
                if (i.customId === 'accept') {
                    ...
                    question.edit({
                        embeds: [
                            embed.setDescription(
                                `Başarıyla ${member} adlı kullanıcının odasına gidildi.`,
                            ),
                        ],
                        components: [],
                    });
                    ...
                    question.edit({
                        embeds: [
                            embed.setDescription(
                                `${member} adlı kullanıcı, ${message.member} kullanıcısının isteğini reddetti.`,
                            ),
                        ],
                        components: [],
                    });
