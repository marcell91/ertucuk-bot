...
module.exports = {
    Name: 'unquarantine',
    Aliases: ['unjail'],
    Description: 'Jail yemiş kullanıcının cezasını kaldırırsınız.',
    ...
    messageRun: async (client, message, args, ertu, embed) => {
        ...
        if (!member.roles.cache.has(ertu.settings.quarantineRole)) {
        ...
        const document = await PunitiveModel.findOne({ user: member.id, active: true, type: 'Quarantine' })
