...
module.exports = async function channelOverwriteDelete(client, guild, audit, member, changes) {
  ...
  const allowedPermissions = changes.find((c) => c.key === 'allow')?.old !== '0' ? new PermissionsBitField(BigInt(changes.find((c) => c.key === 'allow')?.old)).toArray() : [];
  const deniedPermissions = changes.find((c) => c.key === 'deny')?.old !== '0' ? new PermissionsBitField(BigInt(changes.find((c) => c.key === 'deny')?.old)).toArray() : [];

  if (logChannel) logChannel.send({
    embeds: [
      new EmbedBuilder({
        title: 'Kanal Yetkisi Silindi!',
        description: [
          codeBlock('yaml', [
