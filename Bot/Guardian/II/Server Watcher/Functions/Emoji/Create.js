...
module.exports = async function emojiCreate(client, guild, audit, member, changes) {
  ...
  if (safeMode && !safeMode.isWarn) await punish(client, member, safeMode.punishType, 'Emoji Oluşturuldu!');
  else if (!safeMode) await punish(client, member, 3, 'Emoji Oluşturuldu!');
};